...
function CustomTooltip({ active, payload, total }: CustomTooltipProps) {
  ...
  return (
    <div className="bg-bg-elevated border border-border-normal rounded-lg px-3 py-2 shadow-lg text-sm pointer-events-none">
      ...
      <p className="text-text-secondary">{val.toLocaleString()} wins</p>
      <p className="text-text-muted">{((val / total) * 100).toFixed(1)}%</p>
    </div>
  );
}

function VictoryPieChart({ playerData }: VictoryPieChartProps) {
